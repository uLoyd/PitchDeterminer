exports.Tuning = class Tuning {
    ...
    tuningSteps() {
        ...
        this.sounds.forEach((sound, index) =>
            steps.push(
                sound.distanceBetweenNotes(sound, this.sounds[index + 1])
            )
        );
    ...
    isStandard(steps = this.tuningSteps()) {
        ...
        if (steps[keypos] !== 4) return false;
    ...
    isDoubleDrop(steps = this.tuningSteps()) {
        // at least for a 6 string
        const lastNote = steps.pop();
        const firstNote = steps.shift();
        const secondNote = steps.shift();

        if (!steps.every((x) => x === 5)) return false;
