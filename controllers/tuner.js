...
class tuner {
    ...
    // c - cents
    updateSpans(f, n, c) {
        if (f) this.frequency.innerText = "Frequency: " + f;
        if (n) {
            if (!isNaN(n.frequency)) n.frequency = n.frequency.toFixed(2);
            this.note.innerText =
                "Note: " + n.note + " Perfect: " + n.frequency;
        }
        if (c) this.cents.innerText = "Cents: " + c;
    }

    async update(data) {
        //console.log(data);
        await this.movePointer(data.centsError);
        await this.updateSpans(
            data.frequency.toFixed(2),
            data.perfectPitch,
            data.centsError.toFixed(2)
        );
    }
